...
(* "_anonymous" 是匿名章节内部保留 ID，不得作为显式 ChapterTitle；违反时触发 E005。 *)
```
...
| BC27 | **显式章节标题为 `_anonymous`** | 报错并使用隔离的容错章节保留标题与节点，不得与真正匿名章节合并 | 🔴 E005 |
| BC28 | **零章节、零节点或任一空章节** | 允许继续编辑，但所有格式导出均被阻断 | 🔴 E009 |
