...
1. 从干净 VM 快照开始，记录 Windows/VM/显示/语言，校验 installer SHA256 与 `SHA256SUMS.txt`。
...
停止点：任何 hash 不一致、安装路径位于源码工作区、注册项缺失或录屏中断，立即结束本包。hash/安装合同实际偏离为 `FAIL`；权限、VM 或录屏问题为 `BLOCKED`。`NotSigned` 单独记录为发行阻断，不否定其他安装完整性。
