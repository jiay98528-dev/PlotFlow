...
[结构化 UX 母版说明](../design/ux-master/README.md) 是当前页面布局、共享控件与状态的设计入口，[预览](../design/ux-master/index.html) 由同目录 .pen 主文件导出。原始生图图册保留为视觉起点；具体操作细节以可编辑母版及本节目标行为为准。下文保留的数据、会话、无障碍和主题合同继续适用；功能分组与用户文案遵循本节及下方当前交互约定。

- Graph 的显示名为“剧情图”，节点编辑区为“剧情详情”。按剧情内容、玩家选项、选择条件、选择后发生的变化组织操作；高级条件按需展开。
- 在规则编辑处可以就地创建变量；使用整数、小数、是非开关、文本、固定选项、组合数据及使用范围等创作语言。对象变化选具体项目，操作随类型约束，不新增公式语言。
- 普通内容变化保持控件身份稳定。输入草稿与已提交内容区分；中文输入法确认文字不提交表单，无效数字不自动变成 0，不触发整组表单回退源码。
- 表单错误就近解释；只读可选择复制，禁用不可编辑；加载阻止重复提交，未实现取消机制时禁用关闭／取消操作。焦点轮廓可叠加选中、悬停和错误状态。
- 窄窗口保留章节导航，目录和详情抽屉互斥；弹层、按钮与操作区必须位于可用区域内。长内容和空白、填写、处理中、成功、失败及冲突状态见母版映射。

现有桌面页面已按母版改版。后续实现按母版定位页面、按本文维护行为合同，开发状态只更新 progress.md。
...
- 顶部第二行固定显示章节标签栏。
- 左侧目录提供章节和剧情导航、故事信息与变量编辑入口、创作按钮及故事文件夹。故事信息与变量在独立弹层编辑，整理布局与创建操作分组。
...
右侧 Inspector 是 Graph Lab 的当前节点编辑面板，使用静态“剧情详情”上下文标题，不使用只有一个项目的伪标签栏。选中节点后，剧情字段、默认“下一步”与可展开的玩家选项在同一滚动流中出现；选项内的条件与变化按需展开，避免把同一节点的路线操作割裂为另一层语义。

左侧故事设置提供“故事信息｜变量”入口，打开独立弹层，编辑不依赖当前节点选择的故事信息和变量。标签切换保留两个表单的挂载；变量草稿在同一会话切换工作区后继续保留。未保存的变量不被导出或整篇保存静默忽略，操作会返回变量表单，请用户保存变量或取消编辑。未选节点时，这两个全局编辑路径仍必须完整可用。标签遵循标准 roving tabindex 模型：`ArrowLeft` / `ArrowRight` 循环切换并移动焦点，`Home` / `End` 跳到首尾，非激活标签不能因 `tabIndex=-1` 而失去键盘进入路径。

必须支持：

- 故事 title、author 和 engine。engine 下拉值为 `generic | godot | unity | unreal`，界面分别显示独立故事、Godot、Unity、Unreal；`generic` 导出为 JSON 0.2 的 `none`，UI 不展示 JSON 专用值 `none`。
...
- 每条路线行显示选项文本、变量需求与目标预览；存在效果时显示变化摘要。使用低强调的文字和底色，不堆叠高饱和标签。
...
页面和控件状态以 [结构化 UX 母版](../design/ux-master/README.md) 为准。三个内置主题共用布局，使用中性阅读面、细分隔线、浅阴影以及独立的焦点轮廓；冷白与淡紫是默认主题方向。旧的液态玻璃、遥测 HUD 和阶段性装饰描述不再作为目标。

- 宽屏目录 208px、详情 384px；中间画布使用剩余空间。900px 及以下改为目录与详情互斥抽屉，入口至少 44px 命中范围。
- 窗口、面板、章节源码与问题区分别约束滚动，不用整个网页的横向滚动容纳编辑器。
- 卡片、路线与表单保持文字清晰；默认剧情图缩放为 100%，用户缩放和移动视口后不因选中节点跳动。
- 源码区域必须留出可用文本高度，操作栏和错误信息不能覆盖文字。
- Home 完整覆盖窗口，其内容可在窄窗口内滚动；主题中心使用三张并列卡片，窄窗口改为单列。
- 按钮与字段状态继承母版：默认、悬停、按下、焦点、禁用、加载；错误就近解释，只读内容可选择复制。危险操作保持危险语义色，加载阻止重复提交。
- `prefers-reduced-motion: reduce` 关闭非必要动画。布局和状态颜色均使用语义 token，主题不能改写数据行为。

### 7.3 会话与可靠性合同

本节规定 Graph Lab 的会话与可靠性行为。当前实现状态见 progress.md。
...
- 不在界面中解释技术实现或快捷键列表。
- 除品牌、文件格式和引擎名称外，界面不混用中英文。Graph Lab、Inspector、Source Drawer 仅用于技术资料，中文界面分别显示剧情图、剧情详情和章节源码。
...
- 结构化设计入口：[UX 母版说明](../design/ux-master/README.md) 与 [母版预览](../design/ux-master/index.html)。页面布局和控件状态以此对照，原始图片仅保留为设计起点。
- 页面视觉参考：[UX 页面样机图册](assets/graph-editing-ux/gallery.html)。评审或实现对应页面前查看；原图、页面编号及提示词由图册定位。
