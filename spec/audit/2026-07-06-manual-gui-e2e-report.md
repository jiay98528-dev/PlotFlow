...
> Post-report remediation（2026-07-06）：本报告保留原始安装版观察，不回写历史结果。报告发现的 `Continue editing` 最近文件恢复缺陷已在源码层修复，并进入外审 P0/P1 回归范围；Graph Lab 章节标签栏已改为可见命令栏行，并增加章节标签截图 E2E。当前源码验证通过，但这些改动发生在本报告所测安装器之后，必须重新打包、安装并复跑 installed blackbox/manual patrol 后，才能把 GUI-E2E-004 或章节标签体验标记为安装版已复验通过。
