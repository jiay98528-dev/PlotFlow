...
describe('main-process localization', () => {
  it('returns Chinese confirmation and system-open copy', () => {
    ...
    expect(text.productName).toBe('维叙（Fablevia）');
    expect(text.storyFileType).toBe('维叙故事文件');
  ...
  it('returns English confirmation and system-open copy', () => {
    ...
    expect(text.productName).toBe('Fablevia');
    expect(text.storyFileType).toBe('Fablevia Story');
